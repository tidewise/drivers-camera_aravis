...
int main(int argc, const char *argv[])
{
	...
	int startExposure = 100;
	int endExposure = 70000;
	int stepSize = 100;
	int currentExposure = startExposure;
	//int time = 0;
	//int currentExposure = 30000;
	FILE* brightnessLog = fopen("brightnessLog.txt", "w");
	if(!brightnessLog) {
		throw runtime_error("Can't open brightness logfile!");
	}
	while(!shouldExit && endExposure > currentExposure) {
		...
		if(arv_buffer != NULL) {
			if(arv_buffer->status == ARV_BUFFER_STATUS_SUCCESS) {
				...
				cout << "Got image "  << arv_buffer->frame_id << " Brightness: " << brightness << " Exposure: " << currentExposure << endl;
				fprintf(brightnessLog, "%d\t%d\n", currentExposure, brightness);
				
				currentExposure += stepSize;
				//time += 1;
